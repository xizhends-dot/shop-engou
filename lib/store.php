...
/** 個別バナーにタイトル・副題・リンクのいずれかが入力されているか */
function banner_has_content(array $b) {
    return trim((string)($b['title'] ?? '')) !== ''
        || trim((string)($b['subtitle'] ?? '')) !== ''
        || trim((string)($b['link'] ?? '')) !== '';
}

/** フロント表示用（未入力の下書きバナーは除外） */
function banners_for_front() {
    $out = [];
    foreach (banners_load() as $b) {
        if (!banner_has_content($b)) {
            continue;
        }
        if (!store_validate_banner_image_path($b['image'] ?? '')) {
            continue;
        }
        $out[] = $b;
    }
